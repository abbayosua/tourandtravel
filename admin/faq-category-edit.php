...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name) $error = t('Nama kategori wajib diisi');
...
$pageTitle = $isAdd ? t('Tambah Kategori') : t('Edit Kategori');
...
<div class="card border-0 shadow-sm mb-3"><div class="card-body">
    <div class="mb-3"><label class="form-label"><?= t('Nama Kategori') ?></label><input name="name" class="form-control" value="<?=e($item['name']??'')?>" required></div>
    <div class="mb-3"><label class="form-label"><?= t('Urutan') ?></label><input name="sort_order" type="number" class="form-control" value="<?=$item['sort_order']??0?>" min="0"></div>
</div></div>
<button type="submit" class="btn btn-primary w-100"><?= $isAdd ? t('Tambah') : t('Simpan') ?></button>
<a href="faq-category.php" class="btn btn-outline-secondary w-100 mt-2"><?= t('Batal') ?></a>
</div></div></form>
