...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($uid) {
            ...
            $message = $companyId ? sprintf(t('User %s ditautkan.'), $email) : sprintf(t('User %s dilepas.'), $email);
        } else {
            $message = sprintf(t('User %s tidak ditemukan.'), $email);
        }
    }
}
...
$pageTitle = t('Corporate Rates');
require_once 'includes/admin-header.php';
?>
<div class="container-fluid py-4">
    <h4 class="fw-bold mb-4"><i class="bi bi-building me-2"></i><?= t('Corporate Rates') ?></h4>
    ...
    <div class="row g-4">
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body p-3">
                    ...
                    <form method="POST">
                        ...
                        <button type="submit" class="btn btn-primary btn-sm"><?= t('Simpan') ?></button>
                    </form>
                </div>
            </div>
                ...
                <div class="card-body p-3">
                    ...
                    <form method="POST">
                        ...
                        <select name="company_id" class="form-select mb-3" required>
                            <option value=""><?= t('— Lepas afiliasi —') ?></option>
                            <?php foreach ($companies as $c): ?>
                            <option value="<?= (int)$c['id'] ?>"><?= e($c['name']) ?> (<?= (float)$c['discount_percent'] ?>%)</option>
                            <?php endforeach; ?>
                        </select>
                        <button type="submit" class="btn btn-primary btn-sm"><?= t('Tautkan') ?></button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-7">
            <div class="card border-0 shadow-sm">
                <div class="card-body p-3">
                    <h6 class="fw-semibold mb-3"><?= t('Daftar Perusahaan') ?></h6>
                    <?php if (empty($companies)): ?>
                    <p class="text-muted small mb-0"><?= t('Belum ada perusahaan.') ?></p>
                    <?php else: ?>
                    <table class="table table-sm align-middle">
                        <thead><tr><th><?= t('Nama') ?></th><th style="width:130px;"><?= t('Diskon %') ?></th><th><?= t('Status') ?></th><th></th></tr></thead>
                        <tbody>
                        <?php foreach ($companies as $c): ?>
                        <tr>
                            <td><?= e($c['name']) ?></td>
