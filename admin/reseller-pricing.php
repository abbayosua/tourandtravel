...
$pageTitle = t('Harga Reseller');
require_once 'includes/admin-header.php';
?>

<h4 class="fw-bold mb-3"><i class="bi bi-tags me-2"></i><?= t('Harga Reseller per Tour') ?></h4>

<?php if ($msg): ?>
    <div class="alert alert-<?= str_starts_with($msg, 'success') ? 'success' : 'danger' ?> py-2"><?= $m = substr($msg, strpos($msg, ':') + 2); $mm = ['need_price' => t('Tour dan harga wajib diisi.'), 'updated' => t('Harga reseller berhasil diupdate.'), 'added' => t('Harga reseller berhasil ditambahkan.'), 'exists' => t('Tour sudah memiliki harga reseller. Edit yang sudah ada.')]; echo e($mm[$m] ?? $m); ?></div>
<?php endif; ?>
    ...
    <div class="card-body p-4">
        ...
        <form method="POST">
            ...
            <div class="row g-3 align-items-end">
                <div class="col-md-4">
                    <label class="form-label small fw-semibold"><?= t('Tour') ?></label>
                    <select name="tour_id" class="form-select" required>
                        <option value=""><?= t('Pilih tour...') ?></option>
                ...
                <div class="col-md-2">
                    <div class="form-check mt-4">
                        ...
                        <label class="form-check-label small" for="activeCheck"><?= t('Aktif') ?></label>
                    </div>
                </div>
                <div class="col-md-2 d-grid">
                    <button type="submit" class="btn btn-primary"><?= $editData ? t('Update') : t('Tambah') ?></button>
                    <?php if ($editData): ?>
                        <a href="reseller-pricing.php" class="btn btn-outline-secondary btn-sm mt-1"><?= t('Batal') ?></a>
                    <?php endif; ?>
                </div>
            </div>
        </form>
        ...
        <div class="table-responsive">
            <table class="table table-sm table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        <th><?= t('Tour') ?></th>
                        <th class="text-end"><?= t('Harga Normal') ?></th>
                        <th class="text-end"><?= t('Harga Reseller') ?></th>
                        <th class="text-center"><?= t('Min Pax') ?></th>
                ...
                <tbody>
                <?php if (empty($prices)): ?>
                    <tr><td colspan="6" class="text-center text-muted py-4"><?= t('Belum ada harga reseller.') ?></td></tr>
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <span class="badge <?= $p['active'] ? 'bg-success' : 'bg-secondary' ?>"><?= $p['active'] ? t('Aktif') : t('Nonaktif') ?></span>
                        </td>
                        <td class="text-end">
                            <a href="?edit=<?= $p['id'] ?>" class="btn btn-sm btn-outline-primary" title="<?= t('Edit') ?>"><i class="bi bi-pencil"></i></a>
                            <a href="?toggle=<?= $p['id'] ?>" class="btn btn-sm btn-outline-warning" title="<?= t('Toggle') ?>" onclick="return confirm('<?= t('Toggle status?') ?>')"><i class="bi bi-toggle-<?= $p['active'] ? 'on' : 'off' ?>"></i></a>
                            <a href="?delete=<?= $p['id'] ?>" class="btn btn-sm btn-outline-danger" title="<?= t('Hapus') ?>" onclick="return confirm('<?= t('Hapus harga reseller ini?') ?>')"><i class="bi bi-trash"></i></a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
