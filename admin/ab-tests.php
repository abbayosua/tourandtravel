...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="fw-bold mb-0"><i class="bi bi-vector-pen me-2"></i><?= t('A/B Testing') ?></h4>
    </div>
    ...
    <div class="card border-0 shadow-sm mb-3">
        <div class="card-body p-3">
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <span class="badge <?= $t['is_active'] ? 'bg-success' : 'bg-secondary' ?>"><?= $t['is_active'] ? t('Aktif') : t('Nonaktif') ?></span>
            </div>
            <?php $results = abResults($t['test_name']); ?>
            <?php if (empty($results)): ?>
            <p class="text-muted small mb-0"><?= t('Belum ada impresi.') ?></p>
            <?php else: ?>
            <table class="table table-sm table-borderless mb-0" style="max-width: 480px;">
                <thead><tr><th><?= t('Varian') ?></th><th><?= t('Impresi') ?></th><th><?= t('Konversi') ?></th><th><?= t('Rate') ?></th></tr></thead>
                <tbody>
