...
function pdfRemoteImg(string $url, string $attrs = '', int $maxW = 320): string
{
    ...
    if (!is_file($file) || filesize($file) < 500 || (time() - filemtime($file) > 7 * 86400)) {
        $ctx = stream_context_create(['http' => ['timeout' => 8, 'user_agent' => siteName() . '-PDF/1.0']]);
...
function pdfTourHtml(string $title, string $meta, string $coverImgTag, array $days): string
{
    ...
    $h .= '<div class="topbar">' . e(siteName()) . ' &mdash; Tour Itinerary</div>';
    ...
    $tag = trim((string)getSetting('site_tagline', ''));
    $h .= '<div class="brand">' . e(brandText('Generated by TourAndTravel') . ($tag !== '' ? ' &mdash; ' . $tag : '')) . '</div>';
    return $h . '</body></html>';
}
...
function pdfUserHtml(string $title, string $sub, string $userName, array $days): string
{
    ...
    $h = '<html><head><meta charset="utf-8"><style>' . pdfBaseCss() . '</style></head><body>';
    $h .= '<div class="topbar">' . e(siteName()) . ' &mdash; ' . e($userName) . ' | Itinerary</div>';
    $h .= '<h1>' . e($title) . '</h1>';
    ...
    $tag2 = trim((string)getSetting('site_tagline', ''));
    $h .= '<div class="brand">' . e(brandText('Generated by TourAndTravel') . ($tag2 !== '' ? ' &mdash; ' . $tag2 : '')) . '</div>';
    return $h . '</body></html>';
}
...
function pdfStreamDownload(Dompdf $pdf, string $html, string $filename, ?string $cliPath = null): void
{
    ...
    try {
        ...
        $pdf->getCanvas()->page_text(250, 815, 'Page {PAGE_NUM} / {PAGE_COUNT} - ' . siteName(), $font, 8, [0.6, 0.6, 0.6]);
    } catch (Throwable $e) {
    }
