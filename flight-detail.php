...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['book_fl'])) {
    if ($mode!=='flightlist' || !$offer) {
        ...
        $bookingError=t('Silakan login terlebih dahulu.');
    } else {
        $name = trim($_POST['name']??''); $phone = trim($_POST['phone']??''); $email = trim($_POST['email']?? (getUser()['email']??''));
        if (!$name || !$phone) $bookingError=t('Nama dan telepon wajib diisi.');
        else {
            $deep = $offer['deep_link'] ?? null;
            $price = $offer['price'] ?? 0;
            $bookingSuccess = t('Penerbangan FlightList dipilih!') . ' ' . ($deep ? t('Lanjutkan di Kiwi: ') . $deep : t('Harga: ') . '$'.number_format($price,0));
    ...
    if ($mode!=='duffel' || !$offer) {
        ...
        if (!$name || !$phone) $bookingError=t('Nama dan telepon wajib diisi.');
            ...
            if (empty($offerPassengers)) $bookingError=t('Data penumpang tidak tersedia.');
            else {
                ...
                if ($passengersCount !== count($passengersData)) {
                    if ($passengersCount!==1) $bookingError=t('Untuk multi-penumpang, silakan cari dengan jumlah penumpang yang sesuai.');
                }
                if (!$bookingError) {
            ...
            <div class="col-md-8">
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body p-4">
                        ...
                        <?php if ($bookingSuccess): ?><div class="alert alert-success py-2" id="bookingSuccess"><?= e($bookingSuccess) ?><?php if($bookingResult && !empty($bookingResult['deep_link'])): ?><br><a href="<?= e($bookingResult['deep_link']) ?>" target="_blank" class="btn btn-sm btn-success mt-2"><?= t('Buka di Kiwi.com') ?></a><?php endif; ?></div><?php endif; ?>
                        ...
                        <?php if ($bookingSuccess && $bookingResult): ?>
                            <a href="flights.php" class="btn btn-primary w-100"><?= t('Cari Lagi') ?></a>
                        ...
                        <form method="POST" id="flBookingForm">
                            <div class="row g-2">
                                ...
                                <div class="col-md-6"><input type="email" name="email" class="form-control" placeholder="<?= t('Email') ?>" value="<?= e(getUser()['email'] ?? '') ?>" required></div>
                                <div class="col-md-3"><select name="passengers" class="form-select"><option value="1">1 <?= t('Penumpang') ?></option></select></div>
                                <div class="col-md-3 d-grid"><button type="submit" name="book_fl" value="1" class="btn btn-primary fw-semibold"><?= t('Pesan Sekarang') ?></button></div>
                            </div>
                            <small class="text-muted d-block mt-2"><?= t('FlightList: klik Pesan menampilkan link Kiwi.com (affiliate).') ?></small>
                        </form>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body p-4 text-center">
                        <span class="badge bg-success mb-2"><?= t('Live via Duffel (Test Mode)') ?></span>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3 pb-3 border-bottom">
                            ...
                            <span class="fw-bold text-primary fs-5"><?= duffelFormatPrice($offer['total_amount'], $offer['total_currency']) ?><small class="fw-normal fs-6 text-muted">/<?= t('org') ?></small></span>
                        </div>
                        <?php if ($bookingSuccess): ?><div class="alert alert-success py-2" id="bookingSuccess"><?= e($bookingSuccess) ?><?php if($bookingResult): ?><br><small><?= t('Order ID: ') ?><?= e($bookingResult['id']) ?> · <?= t('Ref: ') ?><?= e($bookingResult['booking_reference'] ?? '-') ?></small><?php endif; ?></div><?php endif; ?>
                        <?php if ($bookingError): ?><div class="alert alert-danger py-2" id="bookingError"><?= e($bookingError) ?></div><?php endif; ?>
                        <?php if ($bookingSuccess && $bookingResult): ?>
                            <a href="flights.php" class="btn btn-primary w-100"><?= t('Cari Penerbangan Lain') ?></a>
                        <?php elseif (!isLoggedIn()): ?>
                            <div class="text-center py-3"><p class="fw-semibold mb-2"><?= t('Login untuk Memesan') ?></p><a href="login.php?redirect=<?= urlencode($_SERVER['REQUEST_URI']) ?>" class="btn btn-primary w-100"><?= t('Masuk / Daftar') ?></a></div>
                        <?php else: ?>
                ...
                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body p-4 text-center">
                        ...
                        <div class="d-flex justify-content-center align-items-center gap-4 my-4">
                            ...
                            <div class="text-center"><i class="bi bi-airplane-fill fs-3 text-primary d-block mb-1"></i><span class="text-muted small"><?= e($schedule['duration']) ?></span><div class="text-muted small"><?= formatDate($schedule['departure_date']) ?></div></div>
                        ...
                        <div class="d-flex justify-content-center gap-4 text-muted small mb-3"><span><?= t('Langsung') ?></span><span><?= t('Sisa') ?> <?= $schedule['available_seats'] ?> <?= t('kursi') ?></span></div>
                    </div>
                </div>
                <div class="card border-0 shadow-sm">
                    <div class="card-body p-4">
