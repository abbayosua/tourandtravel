...
if (!in_array($pdfLang, ['id', 'en', 'zh'], true)) $pdfLang = getCurrentLang();
...
$stmt = db()->prepare("SELECT departure_date, return_date, available_slots, booked, price_adult, price_child, price_single, note, note_en, note_zh FROM tour_dates WHERE tour_id = ? AND is_active = 1 AND departure_date >= CURDATE() ORDER BY departure_date LIMIT 12");
...
pdfStreamDownload(pdfNew($pdfLang), $html, $filename, '/tmp/tour_itinerary_output.pdf');
