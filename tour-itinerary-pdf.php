...
$stmt = db()->prepare("SELECT departure_date, return_date, available_slots, booked, price_adult, price_child, price_single, note FROM tour_dates WHERE tour_id = ? AND is_active = 1 AND departure_date >= CURDATE() ORDER BY departure_date LIMIT 12");
$stmt->execute([(int)$tour['id']]);
$deps = $stmt->fetchAll();

$html = pdfTourBrochureHtml($tour, $itineraries, $coverImg, $deps);

$filename = 'tour-itinerary-' . $tour['slug'] . '.pdf';
$filename = substr(preg_replace('/[^a-zA-Z0-9._-]/', '_', $filename), 0, 90) . '.pdf';

pdfStreamDownload(pdfNew(), $html, $filename, '/tmp/tour_itinerary_output.pdf');
