...
$stmt->execute([$slug]);
$post = $stmt->fetch();
if (!$post) { http_response_code(404); $pageTitle = t('Artikel tidak ditemukan'); require_once 'includes/header-shared.php'; echo '<div class="container py-5 text-center"><h3>' . t('Artikel tidak ditemukan') . '</h3><a href="blog.php" class="btn btn-primary mt-3">' . t('Kembali') . '</a></div>'; require_once 'includes/footer-shared.php'; exit; }
